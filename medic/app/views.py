...
def patientprofile(request):
    ...
    appointment = doctor.objects.all()

    return render(request,'profile/patient/profilep.html',{'loguser':log_user,'patients':patientdata, 'form':form, 'post':post, 'appoint':appointment}) 
...
def addpost(request):
    ...
    if request.method == "POST":
        ...
        if fm.is_valid():
            ...
            dra = fm.cleaned_data['draft']
            reg = Doctorpost(user=user,title=tit,image=img,category=cat,summary=summ,content=cont, draft=dra)
            reg.save()
    else:
        fm=DoctorPostForm()
